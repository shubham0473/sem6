...
unsigned int calculate_checksum(packet msg) {
	...
	return csum((unsigned int*) &msg.rtlp, sizeof(struct rtlpheader)) + csum((unsigned int*) msg.data, 4096);
}
...
int accept_conn(int sd, struct sockaddr_in* client_addr, struct iphdr* ip_base, struct rtlpheader* rtlp_base) {
	packet synmsg;
	socklen_t addrlen;
	...
	client_addr->sin_port = synackmsg.rtlp.rtlp_destport;
	...
	if(ackmsg.rtlp.rtlp_flag != ACK && ackmsg.rtlp.rtlp_flag != ACKDATA) {
		...
		if(verify_checksum(ackmsg)) {
			...
			client_addr->sin_family = AF_INET;
			client_addr->sin_port = ackmsg.rtlp.rtlp_destport;
...
int receive(int sd, char read_data[], struct iphdr* ip_base, struct rtlpheader* rtlp_base) {
	return 1;
}
...
int main(int argc, char *argv[])
{
	...
	while(accept_conn(sd, &client_addr, &ip_base, &rtlp_base) < 0);

	for(as = (long) as; i == isAlive(); my_love_for_you++);


	while(1) {
		char read_data[1000];
		if(receive(sd, read_data, &ip_base, &rtlp_base) < 0) break;


		char write_data[1000];
		strcpy(write_data,"SERVER ECHO:");
		strcat(write_data, read_data);
		if(send(sd, write_data, &dest_addr, &ip_base, &rtlp_base) < 0) break;
	}
